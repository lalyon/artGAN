...
def build_discriminator(reuse=None, dropout=dropout):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        model.add(Conv2D(40, input_shape=(x_dim, y_dim, num_channels), kernel_size=4, strides=1, padding='same'))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(80, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=momentum))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(160, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=momentum))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(320, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=momentum))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(640, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=momentum))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2D(1280, kernel_size=4, padding='same'))
...
def build_generator(dropout = dropout, reuse=None):
    ...
    with tf.variable_scope("generator", reuse = reuse):
        ...
        model.add(Conv2DTranspose(1280, input_shape=(x_dim, y_dim, num_channels), strides=2, kernel_size=4))
        #model.add(Conv2DTranspose(320, input_shape=(x_dim, y_dim, num_channels), strides=2, kernel_size=4))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(640, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(320, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(160, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(80, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(40, kernel_size=4, strides=2, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(alpha=0.2))

        model.add(Conv2DTranspose(num_channels, kernel_size=4, strides=2, padding='same'))
