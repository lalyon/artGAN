...
newdir = "modifiedImages128Paintings"
...
def generator(input_noise, dropout = dropout, reuse = None, is_training = is_training):
    momentum = 0.92 #could be tweaked
...
for epoch in range(num_epochs):
    ...
    if training_discriminator:
        ...
        if not epoch % 100:
            save_path_string = "./paintingsCheckpoints/d_models/model.ckpt"
    ...
    if training_generator:
        ...
        if not epoch % 100:
            save_path_string = "./paintingsCheckpoints/g_models/model.ckpt"
            save_path = saver.save(sess, save_path_string, global_step=epoch)
    if not epoch % 10:
