-- modification of Robbie Barrat's Github code via https://github.com/robbiebarrat/art-DCGAN
...
if (opt.netG ~= '') then
  ...
  -- nInputPlane = "number of expected input planes in the image"; "number of output planes the convolution layer will produce."
  -- kW, kH = kernel width/height, resp.
  -- dW, dH = "step of the convolution in the [...]] dimension" width/height, resp.
...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % 1 == 0 and opt.display then
          ...
          local real = data:getBatch()
          disp.image(fake, {win=opt.display_id, title=opt.name .. "epoch_" .. epoch .. "_" .. counter})
