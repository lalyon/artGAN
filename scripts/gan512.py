...
#
# for dir, _, files in os.walk(parent_dir_2):
#     for filename in files:
#         relDir = os.path.relpath(dir, parent_dir_2)
#         relFile = os.path.join(relDir, filename)
#         filepaths.append(parent_dir_1 + "/" + relFile)
#
...
def discriminator(input_image, reuse = None, dropout=dropout):
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        x = tf.layers.dense(x, units=128, activation = leakyRelu)
...
for epoch in range(num_epochs):
    ...
    if gen_ls * 1.35 < discrim_ls:
        training_generator = False
        pass
    if discrim_ls * 1.35 < gen_ls:
    ...
    if training_discriminator:
        ...
        # if not epoch % 100:
        #     save_path_string = "./512checkpoints/d_models/model.ckpt"
        #     save_path = saver.save(sess, save_path_string, global_step=epoch)
    if training_generator:
        sess.run(opt_gen, feed_dict={noise: n, dropout: dropout_training, is_training:True})
        # if not epoch % 100:
        #     save_path_string = "./512checkpoints/g_models/model.ckpt"
        #     save_path = saver.save(sess, save_path_string, global_step=epoch)
    if not epoch % 10:
